...
def encrypt(public_key, hex_number):
    ...
    message = (hex_number ** public_key[0]) % public_key[1]
    return message
    pass
...
def decrypt(private_key, encrypted_msg):
    message = (encrypted_msg ** private_key[0]) % private_key[1]
    return message
    pass
    ''' complete with necesarry code '''
...
def number_check(hex_nr):
    hex_nr = int(hex_nr, 16)
    haxx = (hex_nr >> 8) & 0xff
    if haxx == 0xfe and low_check(hex_nr):
        return True
    return False
    pass
    ''' complete with necesarry code '''

# print(number_check(69))
# print(low_check(51))
# keys = generate_keypair(prime_number_1,prime_number_2)
# #
# # print(hex(int("0x3578",16)))
# encrypted = encrypt(keys[0],hex(int("0xfd02",16)))
# print(encrypted)
